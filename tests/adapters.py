...
from cs336_basics.model.attention import CausalMultiHeadSelfAttention, scaled_dot_product_attention
...
def run_rmsnorm(
    d_model: int,
    eps: float,
    weights: Float[Tensor, " d_model"],  # type: ignore[name-defined]
    in_features: Float[Tensor, " ... d_model"],
) -> Float[Tensor, " ... d_model"]:
...
def run_cross_entropy(
    inputs: Float[Tensor, " batch_size vocab_size"],  # type: ignore[name-defined]
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    return train_tokenizer(input_path=input_path, vocab_size=vocab_size, special_tokens=special_tokens)
